...
LexScan::LexScan() {
	...
	for (int i=0;i<keywordnum;i++)
	{
		...
		cout << "get word ["<<keywordtoken[i]<<"]:"<<keyword[i]<<endl;
	}
	for (int j=0;j<opnum;j++)
	{
		fscanf(fp,"%s",operat[j]);
		fscanf(fp,"%d",&operattoken[j]);
		cout << "get word ["<<operattoken[j]<<"]:"<<operat[j]<<endl;
	}
	fclose(fp);
}
...
int LexScan::Lexical() {
	...
	if (getword(&str[cur])){//is keyword or word
		checkKeyword();// if is keyword, change the token
		return 0;
...
int LexScan::getOperater(const char *str) {
	...
	for (int i=0;i<opnum;i++)
	{
		...
		{
			id[0]=str[0];
			if (!isa(str[1])&&!isnum(str[1])&&!isntrs(str[1]))
			{
			// is operater
				if (str[1]==operat[i][1])
				{
